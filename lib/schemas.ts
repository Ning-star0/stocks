...
const newsEvidenceCoverageSummarySchema = z.object({
  ...
  sharedTopicKey: z.string().nullable().optional(),
  sharedTopicSource: z.enum(["alias_map_v1", "verified_industry_v1"]).nullable().optional(),
  sharedTopicReused: z.boolean().optional(),
  industryClassification: z.object({
    schemaVersion: z.literal("news-industry-classification-v1"),
    status: z.enum(["verified", "missing", "stale", "conflicted"]),
    symbol: z.string(),
    industryName: z.string().nullable(),
    provider: z.literal("EASTMONEY").nullable(),
    classificationMethod: z.literal("EASTMONEY_EM2016").nullable(),
    classificationSourceUrl: z.string().nullable(),
    fetchedAt: z.string().nullable(),
    validUntil: z.string().nullable(),
    maximumAgeHours: z.number().positive(),
    sourceEvidenceHash: z.string().nullable(),
