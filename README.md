...
- **AI 研究与纪律助手** — AI 阅读持仓、技术指标、公告、新闻和交易记忆并形成多空解释；最终状态、价格、费用、仓位和风险由服务端结构化规则复核
- **证据优先决策** — 每次分析携带版本化证据包、最近 60 根结构化 K 线、确定性市场特征和数据质量报告；缺关键披露、财务、时效或新闻验证时，只能输出“证据不足”，不得形成买入计划
- **标的类型证据政策** — 先按交易所代码规则区分 A 股公司、ETF、指数和未知标的。A 股公司使用财务/法定公告链；ETF 专属产品资料、管理人公告、跟踪质量、流动性和折溢价链尚未接入完整，因此会明确显示“ETF 产品证据未闭合”，不会误报为缺少上市公司财务，也不会绕过风控
- **结构化组合决策** — 聚焦决策只接受单股 `decisionStatus`、`dataQuality`、服务端 `tradePlan`、确定性量化/费用/风险门控；AI 趋势、置信度和中文建议不能创建、取消或扩大订单
- **影子观察与真实计划分离** — 数值条件完整但概率尚未校准的方案只进入不可执行影子区，展示触发、止损、目标、费用和风险，不占资金、不进入订单、不触发买入推送
...
| AI | OpenAI 兼容 API（DeepSeek v4 Flash / Pro 成本感知路由） |
...
| `OPENAI_MODEL` | 高风险复核模型（Pro） | `deepseek-v4-pro` |
| `OPENAI_STANDARD_MODEL` | 常规研究模型（Flash） | `deepseek-v4-flash` |
...
当前服务器配置为 09:30 抓取新闻，10:45、14:30 各执行一次关注标的证据刷新；这两个分析时点都处于 DeepSeek 高峰计费时段。系统保留盘中时效，但把证据刷新与模型重算分开：行情、新闻、公告和基本面照常刷新，只有价格跨越 3%、技术状态变化、新增高影响新闻、证据质量/公告/基本面发生实质变化、用户持仓或风险变化、重要提醒触发，或上次分析超过 6 小时时才再次调用模型。因此正常情况下 10:45 产生一次分析，14:30 多数标的复用；剧烈行情或重要新证据出现时仍会重算。

模型职责固定如下：

| 场景 | 模型 | 原因 |
|---|---|---|
| 未持仓个股研究、新闻精读/检索规划、聚焦组合解释、日报、预测、问答 | `deepseek-v4-flash` | 高频、可由 schema 和确定性代码复核 |
| 已有持仓的单股复核 | `deepseek-v4-pro` | 涉及真实持仓风险，保留更强推理预算 |
...
`update.sh` 仅在确认工作目录为 `/opt/stocks` 后操作；拉取代码前会检查 Tesseract 及 `chi_sim`、`eng` 语言包，缺失时停止部署，避免扫描公告静默不可读。迁移后、清理旧构建前会运行隔离数据库端到端验收；基线漂移或任一数据库测试失败都会停止部署并保留当前运行服务。健康检查失败时会输出两个服务的最近日志并以失败状态退出。
...
数据库端到端测试必须通过上述命令在一次性 PostgreSQL schema 中运行。脚本先将当前 `prisma/schema.prisma` 重新生成的 SQL 与已审查的 `prisma/baseline/current-schema.sql` 比较，漂移时在创建数据库对象前失败；一致时创建随机 `codex_e2e_*` schema、应用基线、执行额度锁/证据持久化/影子预测测试，并在 `finally` 中级联删除。禁止直接把 `RUN_DB_E2E_TESTS=true` 指向生产 `public` schema，因为影子预测后台刷新会扫描待处理记录。生产升级仍只允许 `prisma migrate deploy`；隔离基线不是生产迁移，也没有修改任何已执行历史迁移的内容或校验和。

该验收覆盖并发额度锁、持久化缓存与过期、显式关键核验、用户级分析复用隔离、交易记忆导致的上下文失效、新闻预期持久化、持久化证据不足继续阻断条件买入、影子预测原子落库和行情失败审计。2026-08-27 服务器固定样本结果为 7/7 通过，且临时 schema 已清理。
...
AI 分析缓存统一使用 `ai_analysis:v19:{userId}:{symbol}:{contextHash}`，按用户隔离。上下文哈希忽略单纯的快照生成时间，但包含财务期、财务内容与派生现金流质量指标、扣非利润法定报告 ID/原文哈希/解析器版本、历史估值价格序列哈希与算法版本、同行行业/样本/分位/跨源核对/证据哈希、新闻行业分类状态/来源/时效/证据哈希、公告原文哈希与提取回执、新闻精读内容、事件时间线/预期回执/价格反应、ETF 专属证据、近期 K 线、沪深 300 市场环境证据、行情修订、组合风险预算、资金和交易记忆；任何实质证据变化都会失效。历史已完成任务不会阻止失败兜底后的再次分析，强制刷新也不会被旧任务吞掉。

ETF 使用版本化 `etf-evidence-v1`。系统目前可以从交易所代码、行情名称和严格截断到分析时点的日 K 生成产品身份与 20 日成交量/成交额代理，但成交额代理只用于同源比较，不冒充已核验人民币成交额。跟踪指数、基金规模、NAV/IOPV、折溢价、持仓暴露、管理人公告、盘口价差和申赎状态尚未接入，因此 ETF 继续处于 `insufficient_data`，不会形成新增买入计划。
...
符合完整证据、风险预算、止损和目标约束的计划会进入版本化 `shadow-forecast-v1` 观察，而不会转成真实买入。每次分析额外读取沪深 300 未复权日线，保存来源 URL、截止时间、特征和证据哈希；宽基数据缺失或超过 7 天时明确标为 `unknown`，AI 不得根据常识或情绪补造市场环境。后台从分析后的下一完整交易日开盘开始，使用未复权日线并严格按当前检查时点截断，观察波段 20 个或长期 63 个交易日内止盈是否先于止损；同日同时触发按止损处理，价格出现无法解释的公司行动时样本作废，行情失败时保留为等待并记录失败原因。样本 cohort 同时使用确定性的沪深 300 市场环境和标的价格环境 `risk_on/neutral/risk_off/unknown`，不使用 AI 趋势文字。校准只统计最新的同模型、同 schema 和同算法版本，禁止把旧模型样本混入；同时按预测时间固定使用最早 70%/最新 30% 的时间留出报告，不能看完结果再调整边界。策略实验室显示样本数、Brier Score、基准 Brier、ECE、概率分箱、时间留出结果、扣费后平均结果，以及等待完整 20/63 日后按相同股数和双边费用计算的同期买入持有基准与超额收益。即使达到最低 100 个已结算样本，当前版本仍为 `shadow_only`；在完成真正独立的冻结测试集、行业环境、公司行动/现金分红和组合级基准前不会解锁买入。
...
批量新闻刷新只在两种情况下共享行业主题请求：命中版本化静态行业别名，或读到当前用户已保存、来源 URL 与股票代码核对通过、且不超过 24 小时的东方财富 EM2016 行业分类。后者使用 `news-industry-classification-v1` 保存行业、来源、抓取时间、有效期和证据哈希；手动关注、定时关注、批量新闻 API、后台批量任务和维护脚本使用同一规则。分类缺失、过期、未来时间、股票不匹配或来源不合格时不得猜测行业，必须退回单股主题查询并在分析页面展示原因。行业分类仅用于减少重复请求，不会解除新闻完整度、额度、关键精读或其他买入硬门控。
