...
10a. 候选若带有 strategyHealth，这是同本金、无未来数据回测产生的 12 小时动态门控。只有足够样本且严重失效时 entryPermission 才会是 pause，此时禁止生成新买入或增持；entryPermission=reduce_size 时买入预算必须减半；卖出、减仓和硬止损不受该门控限制。ranking 必须写明样本外净收益、回撤和门控原因。
